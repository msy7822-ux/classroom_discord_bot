...
4. Cron定期実行の設定
   ...
   - デフォルト: 毎週火曜日と金曜日のJST 9時（UTC 0時）
   - Cron形式: `分 時 日 月 曜日`（UTC時間）
   - 注意: Cloudflare WorkersのCronはUTCで実行されるため、JSTに合わせて調整が必要
   - 例: `0 0 * * 1` = 毎週月曜日UTC 0時（火曜日JST 9時）、`0 0 * * 4` = 毎週木曜日UTC 0時（金曜日JST 9時）
...
- JST（UTC+9）で実行する場合のUTC時間の計算:
  - JST 9時 = UTC 0時（前日の24時）
  - 例:
    - `0 0 * * 1` - 毎週月曜日UTC 0時 = 火曜日JST 9時
    - `0 0 * * 4` - 毎週木曜日UTC 0時 = 金曜日JST 9時
    - `0 0 * * 0` - 毎週日曜日UTC 0時 = 月曜日JST 9時
